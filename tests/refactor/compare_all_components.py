#!/usr/bin/env python3
"""Run all Level 2 component tests and compare against reference outputs.

Finds all component scenarios under tests/refactor/component/, runs SUMO on each,
saves outputs to current/ subdirectory, and compares current/ vs reference/ using
the compare_outputs module.

Usage:
    python compare_all_components.py [--sumo-binary path]
"""
...
def find_component_scenarios(base_dir):
    """Find all .sumocfg files under the component/ subdirectory.

    Parameters
    ----------
    base_dir : str
        Base directory containing the component/ subdirectory.
...
def main():
    parser = argparse.ArgumentParser(
        description="Run all Level 2 component tests and compare against references."
    )
    parser.add_argument(
    ...
    print("=" * 60)
    print("Level 2: Component Tests")
    print("=" * 60)
    print(f"  SUMO binary: {args.sumo_binary}")
    print(f"  Base dir:    {args.base_dir}")
    print()

    scenarios = find_component_scenarios(args.base_dir)

    if not scenarios:
        print("No component scenarios found!")
        sys.exit(1)

    print(f"Found {len(scenarios)} component scenario(s):\n")
    ...
    print()
    print("=" * 60)
    print(f"Component Test Results: {pass_count} PASS, {fail_count} FAIL, {skip_count} SKIP")
    print("=" * 60)
